...
def _fallback_candidates(
    ...
    templates = [
        ...
        (
            ...
            "不复用外部已有专利中的完整能耗控制模型、执行机构配置或既有权利要求步骤组合。",
            "将项目材料中的能效指标、稳定性约束和反馈修正机制组合为新的控制生成方法。",
            "技术效果来自指标建模、约束校验和反馈修正之间的联动。",
            "若现有专利已公开相同指标体系和闭环控制步骤，需人工确认保护边界。",
        ),
        (
            f"{domain}异常诊断系统",
            ...
            "不复用外部已有专利中的固定故障规则库、单点报警结构或既有诊断模块组合。",
            "将项目材料中的数据清洗、特征构建、异常归因和处置建议形成连续诊断链。",
            "技术效果来自异常归因和处置建议联动带来的可解释诊断能力。",
            "若相似专利已覆盖同类异常特征和归因规则，需核查权利要求重合度。",
        ),
        (
            f"{domain}数字孪生监控系统",
        ...
        (
            ...
            "不复用外部已有专利中的固定参数校准流程、既有优化器结构或完整控制步骤组合。",
            "将项目材料中的实时偏差、历史误差和运行约束作为参数校正触发与保护机制。",
            "技术效果来自偏差识别、参数校正和约束保护的联动。",
            "若现有专利已覆盖相同校正触发条件和参数更新方式，需人工确认。",
        ),
    ]
    return [
        PatentCandidate(
            ...
            summary=(
                ...
                f"创新点：{innovation}\n"
                f"避让现有技术：{avoidance}\n"
                f"未复用已有技术特征：{non_reused}\n"
                f"新技术特征：{new_features}\n"
                f"技术效果来源：{effect_source}\n"
                f"重合风险：{risk}\n"
                f"人工确认点：需核验正式专利库中的权利要求、实施例和同族专利。\n"
